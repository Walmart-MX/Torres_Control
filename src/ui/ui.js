 ...
 *
 * Nota sobre la dependencia hacia Events:
 *   renderSVE() adjunta un listener a #btnForceExport que llama
 *   Events.handleForceExport(). renderCatalog() usa Events.delOp()
 *   en onclick inline. Ambas referencias funcionan porque Events sigue
 *   en el scope del IIFE en index.html. Se resolverán cuando Events
 *   se extraiga en la Fase 11.
 */
    ...
    tbody.innerHTML = [...State.catalog.entries()]
      ...
      .map(([op, lic]) => `
        <tr>
          ...
          <td><button class="btn-del" onclick="Events.delOp(${JSON.stringify(op)})">✕</button></td>
        </tr>`).join('');
  },
